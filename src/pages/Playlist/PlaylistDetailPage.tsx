...
const PlaylistDetailPage = () => {
	...
	const { data: playlistItems, error: playlistError, isLoading: playlistIsLoading, hasNextPage, isFetchingNextPage, fetchNextPage } = useGetPlaylistItems({ playlist_id: id ?? "", limit: PAGE_LIMIT, offset: 0 });
						...
						<TableBody>
							{playlistItems?.pages.map((page, pageIdx) =>
								page.items.map((item, itemIdx) => {
									return <DesktopPlaylistItem item={item} key={pageIdx * PAGE_LIMIT + itemIdx + 1} index={pageIdx * PAGE_LIMIT + itemIdx + 1} />;
								})
							)}
						</TableBody>
					</Table>
